...
function MovieDetails() {
  ...
  const { movie, getSeason, getCollection } = useContext(MovieContext);
  const { loading } = useContext(LoadingContext);

  // NOTES -- EVERY TIME MOVIE CHANGES IN GLOBAL STATE SEND FETCH REQUEST AND GET MOVIE CREDIT DETAILS
  useEffect(() => {
    ...
    <div className="relative w-full overflow-x-hidden ">
      ...
      <div className="w-full mb-16 ">
        <div className="flex items-center justify-between w-full px-8">
          {/* NOTES BOTH SECTIONS -- GENRE / RATING / SEASONS */}
        ...
        <div className="flex items-center px-4 py-4">
          <h2 className="px-8 text-xl">{movie.overview}</h2>
        </div>
        <div className="px-12">
              ...
              <div className="mb-4">
                ...
                {movie.seasons &&
                  movie.seasons.map((season) => {
                    return (
                      <Link
                        to={`/seasondetails/${movie.id}/${season.season_number}`}
                      >
                        <div
                          className="flex items-center w-full  mt-4 rounded-xl hover:bg-gradient-to-r hover:from-cyan-500 hover:to-blue-500  hover:transition-all cursor-pointer"
                          key={season.id}
                          onClick={() =>
                            getSeason(movie.id, season.season_number)
                          }
                        >
                          <img
                            src={`${BASE_IMAGE_URL}${
              ...
              <div className="mb-4">
                ...
                {movie.belongs_to_collection && (
                  <>
                    ...
                    <Link
                      to={`/collectiondetails/${movie.belongs_to_collection.id}`}
                    >
                      <img
                        src={`${BASE_IMAGE_URL}${
          ...
          {/* MAP CAST AND CREW MEMBERS FOR MOVIE && TV */}
          <div className="">
            <div className="flex flex-col">
          ...
          {/* NOTES -- WORK ON THIS SECTION */}
          {/* MAP EXTRA DETAILS FOR MOVIE && TV */}
          {movie.homepage && (
            <>
          ...
          <h1 className="text-3xl mb-4 border-b-2 mt-4 border-b-blue-500 w-fit">
            ...
            movie.production_companies.map((company) => {
              return (
                <div className="flex items-center" key={company.id}>
                  <img
                    src={`${BASE_IMAGE_URL}${
                      company.logo_path ? company.logo_path : movie.poster_path
                    }`}
